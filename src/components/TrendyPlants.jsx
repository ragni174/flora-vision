  ...
  return (
    <section className="py-16 px-6 sm:px-12 lg:px-12 xl:px-20 relative z-10 bg-transparent overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto">
        ...
        {/* Header */}
        <div className="flex justify-center mb-12 lg:mb-16 relative">
          <motion.div
        ...
        <div className="flex flex-col gap-8 lg:gap-40 mt-8 lg:mt-20">
          {sections.map((section) => (
            ...
            >
              ...
              {/* ====== MOBILE / TABLET CARD (hidden on lg+) ====== */}
              <div className="lg:hidden atmospheric-box bg-gradient-to-br from-white/[0.05] via-[#0d120d]/70 to-[#0d120d]/90 border border-white/10 rounded-[32px] sm:rounded-[48px] overflow-hidden group hover:-translate-y-2 transition-all duration-700">
                {/* Plant image in normal flow — no overlap */}
                <div className="flex justify-center pt-8 pb-2">
                  <img
                    src={section.imageSrc}
                    alt={section.title}
                    className="w-[52%] sm:w-[42%] max-w-[240px] h-auto object-contain drop-shadow-[0_20px_40px_rgba(0,0,0,0.6)] group-hover:scale-105 transition-transform duration-700"
                  />
                </div>
                {/* Text below */}
                <div className="px-6 sm:px-10 pb-8 text-center">
                  <h3 className="text-xl sm:text-2xl font-sans font-bold text-white mb-3 leading-[1.1] tracking-tight">
                    {section.title}
                  </h3>
                  <p className="text-[13px] sm:text-[14px] text-gray-300 mb-5 leading-relaxed font-light max-w-sm mx-auto">
                    {section.subtitle}
                  </p>
                  <div className="text-[26px] sm:text-[30px] font-sans text-white font-medium mb-6 drop-shadow-md">
                    {section.price}
                  </div>
                  <div className="flex items-center justify-center space-x-4">
                    <button className="px-7 py-2.5 rounded-xl border border-white/30 text-white hover:bg-white hover:text-black transition-all duration-500 text-sm font-semibold tracking-wider">
                      Explore
                    </button>
                    <button className="w-11 h-11 rounded-xl border border-white/30 flex items-center justify-center hover:bg-white hover:text-black transition-all duration-500 text-white">
              ...
              {/* ====== DESKTOP CARD (hidden below lg) — Original Breakout Design ====== */}
              <div className={`hidden lg:flex atmospheric-box bg-gradient-to-br from-white/[0.05] via-[#0d120d]/70 to-[#0d120d]/90 border border-white/10 rounded-[60px] xl:rounded-[100px] h-[400px] xl:h-[500px] relative items-center group hover:-translate-y-4 transition-all duration-700 will-change-transform`}>
                ...
                {/* Plant Image — Breakout */}
                <div className={`absolute -top-32 ${section.reversed ? '-right-8' : '-left-8'} ${section.lgImageWidth} drop-shadow-[0_30px_50px_rgba(0,0,0,0.7)] z-20 group-hover:scale-105 transition-transform duration-1000 group-hover:brightness-110`}>
                ...
                <div className={`w-1/2 flex flex-col justify-center px-16 xl:px-24 z-10 ${section.reversed ? 'text-left' : 'ml-auto'}`}>
                  <h3 className="text-4xl xl:text-5xl font-sans font-bold text-white mb-6 leading-[1.1] tracking-tight">
                    {section.title}
                  </h3>
